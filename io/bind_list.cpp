...
public:
	virtual TCHAR* getdescription(int index)
	{
	...
	virtual bool getbutton(int which, int16_t & value, int & retro_id, bool & isanalog)
	{
		...
		if (retro_id == 17 || retro_id == 19)
		{
			//fix for xinput
			if (b.e.type == dinput::di_event::ev_xinput && b.e.joy.type == dinput::di_event::joy_axis &&
				b.e.xinput.type == dinput::di_event::xinput_axis && b.e.joy.which == 1)
			{
				isanalog = true;
			}
	...
	virtual void replace(unsigned index, const dinput::di_event & e, unsigned action, TCHAR* description, unsigned retro_id)
	{
		...
		for (int i = 0; i < (list2.end() - list2.begin()); i++)
		{
			...
			{
				bind list_binds = list2.at(i);
				b.value = 0;
				b.e = list_binds.e;
				b.action = list_binds.action;
				b.retro_id = list_binds.retro_id;
				lstrcpy(b.description, list_binds.description);
			}
					...
					if ( it->joy.type == dinput::di_event::joy_axis )
					{
						for ( itb = list.begin(); itb < list.end(); ++itb )
						{
							if ( itb->e.type == dinput::di_event::ev_joy &&
								...
								itb->e.joy.which == it->joy.which )
							{
								if ( it->joy.axis != itb->e.joy.axis && itb->e.joy.axis != dinput::di_event::axis_center) press( itb->action, it->joy.value );
							}
						}
						for ( itb = list.begin(); itb < list.end(); ++itb )
						{
					...
					else if ( it->joy.type == dinput::di_event::joy_pov )
					{
						for ( itb = list.begin(); itb < list.end(); ++itb )
						{
							if ( itb->e.type == dinput::di_event::ev_joy &&
								...
								itb->e.joy.which == it->joy.which )
							{
								if ( it->joy.pov_angle != itb->e.joy.pov_angle ) press( itb->action, 0 );
							}
						}
						for ( itb = list.begin(); itb < list.end(); ++itb )
						{
					...
					else if ( it->xinput.type == dinput::di_event::xinput_trigger )
					{
						for ( itb = list.begin(); itb < list.end(); ++itb )
						{
								...
								itb->e.xinput.which == it->xinput.which )
							{
								...
								else press( itb->action, it->xinput.value * 257 );
							}
						}
					}
				}
